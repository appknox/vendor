...
class MuxConnection(object):
    ...
    def _processpacket(self):
        ...
        if resp == self.proto.TYPE_DEVICE_ADD:
            self.devices.append(MuxDevice(
                data['DeviceID'], data['Properties']['ProductID'],
    ...
    def _exchange(self, req, payload={}):
        ...
        if recvtag != mytag:
            raise MuxError("Reply tag mismatch: expected %d, got %d" % (
                mytag, recvtag))
        return data['Number']
    ...
    def process(self, timeout=None):
        ...
        rlo, wlo, xlo = select.select(
            [self.socket.sock], [], [self.socket.sock], timeout)
        if xlo:
