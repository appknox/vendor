...
# ############################# AXML FORMAT ##################################
# Translated from http://code.google.com/p/android4me/source/browse/src/
# android/content/res/AXmlResourceParser.java
    ...
    def getXMLNS(self):
        ...
        for i in self.m_uriprefix:
            if i not in self.visited_ns:
                buff += "xmlns:%s=\"%s\"\n" % (
                    self.sb.getRaw(self.m_uriprefix[i]),
                    self.sb.getRaw(self.m_prefixuri[self.m_uriprefix[i]]))
...
class AXMLPrinter:
    ...
    def complexToFloat(self, xcomplex):
        return (float)(xcomplex & 0xFFFFFF00) * \
            RADIX_MULTS[(xcomplex >> 4) & 3]
